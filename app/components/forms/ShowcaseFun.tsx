...
export default function ShowcaseFun() {
  ...
  const fetchFun = useCallback(async () => {
    const response = await fetch("/api/mcpclient-fun", { method: "GET" });
  ...
  const loadFuns = useCallback(async () => {
    setLoadingTools(true);
    setError(null);

    try {
      const nextFun = await fetchFun();
      console.log("Fetched tools:", nextFun);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/mcpclient-fun", {
        method: "POST",
  ...
  return (<>
    <button type="button" onClick={loadFuns}>
       fun
        </button>
    <form
    ...
    >
      <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
        ...
        <button type="button" onClick={loadFuns} disabled={loadingTools}>
